...
### Download Hosting
Private repos can't host public downloads, so installers are uploaded to this repo's GitHub Releases with tag-specific releases:
- `EliteDraft-latest` → `EliteDraft-Setup.exe`
- `EliteScan-latest` → `EliteScan-Setup.exe`
- `EliteCoord-latest` → `EliteCoord-Setup.exe`
- `EliteCore-latest` → `EliteCore-Setup.exe`
